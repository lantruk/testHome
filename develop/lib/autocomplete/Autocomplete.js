...
import {AutocomleteCustomScroll} from 'lib/cunstomScrollBar';
...
const factory = (Chip, Input) => {
    class Autocomplete extends Component {
        static propTypes = {
            ...
            externalDepence: PropTypes.bool,
        ...
        componentDidMount() {
            //Fix для apple где не срабатывает блюр автоматически
            window.document.addEventListener('touchstart', this.handleDocumentMouseDown)
        ...
        handleDocumentMouseDown = () => {
            if (this.state.focus && !events.targetIsDescendant(event, ReactDOM.findDOMNode(this))) {
                this.refs.autocomlete.querySelector('input').blur()
        ...
        handleQueryBlur = (event) => {
            ...
            if (this.props.onBlur) this.props.onBlur(this.state.query, event)
        };
        ...
        handleQueryFocus = (event) => {
            ...
            if (this.props.onFocus) this.props.onFocus(this.state.query, event)
        };
        ...
        suggestions() {
            let suggest = new Map();
            //debugger
            const query = this.state.query.title ? this.state.query.title.toLowerCase().trim() || '' : this.state.query.toLowerCase().trim() || '';
            const values = this.values();
            const source = this.source();
            console.log(query, values, source)
        ...
        renderSuggestions() {
            const {theme, scroll} = this.props;
            const ulClassName = classnames(theme.suggestions, {[theme.up]: this.state.direction === 'up'}, {[theme.__inScroll]: scroll});

            if (this.props.showLoading && this.props.isLoading) {
                return (
                    <ul ref='suggestions' className={ulClassName}>
                        <li className={classnames(theme.suggestion, theme.loader)}>
                            <Loader multicolor/>
                        </li>
                    </ul>
                )
            ...
            if (scroll) {
                return (
                    <div className={classnames(theme.scrollContainer,{[theme.active]: this.state.focus})}>
                        <AutocomleteCustomScroll active={this.state.focus} topPosition={0}>
                            <ul ref='suggestions' className={ulClassName}>{suggestions}</ul>
                        </AutocomleteCustomScroll>
                    </div>
                );

            } else {
                return <ul ref='suggestions' className={ulClassName}>{suggestions}</ul>;
            }
        }
        ...
        render() {
            ...
            return (
                <div data-react-toolbox='autocomplete' ref='autocomlete' className={className}>
                    {this.props.selectedPosition === 'above' ? this.renderSelected() : null}
                    <Input
                        //{...other} //заблочил ибо проходят дальше ненужные свойства
                        ref='input'
