...
export class GenericScrollBox extends React.Component {
    ...
    _forceSync() {
        ...
        if (this.scrollX == 0 || this.scrollX == this.scrollMaxX) {
            if (this.scrollX == 0) {
                if (this.refs.left.classList.length > 2) {
                    this.refs.left.classList.remove('active')
                }
            }

            if (this.scrollX == this.scrollMaxX) {
                if (this.refs.right.classList.length > 2) {
                    this.refs.right.classList.remove('active')
                }

            }
        } else {
            if (this.refs.left.classList.length == 2 || this.refs.right.classList.length == 2) {
    ...
    onDragStart_chart(e, axis) {
        ...
        let onDrag = e => {
            ...
            if (axis == X) {
                var x = this.scrollMaxX * (OFFSET_X - e.clientX) / this.trackMaxX;
            } else {
                var y = this.scrollMaxY * (e.clientY - OFFSET_Y) / this.trackMaxY;
            }
            this.scrollTo(x, y, 0);
        ...
        let onDragEnd = e => {
            ...
            if (this._forceSyncId) {
                ...
                this.refs.statistic.classList.remove('statistic--dragged')
                //this.refs.statistic.classList.remove('statistic--canMoved')
        ...
        this.refs.statistic.classList.add('statistic--dragged')
    };
    ...
    onClickButton(e, num) {
        ...
        const OFFSET_X = +this.trackX.querySelector('.scroll-box__handle--x').style.left.split('px')[0]
        var x = this.scrollMaxX * (OFFSET_X + 50 * num) / this.trackMaxX;

        this.scrollTo(x, 0, 0);
    }

    onDragStartX = e => this.onDragStart(e, X);
    onDragStart__chart = e => this.onDragStart_chart(e, X);
    onClickButton__chart = num => e => this.onClickButton(e, num)
    ...
    cach = (e) => {
        this.onDragStart__chart(e)
       /* e.stopPropagation()
        e.preventDefault()
        console.log('меняем сласс')
        this.refs.statistic.classList.add('statistic--canMoved')*/
    }
    ...
    render() {
        ...
        return (
            <div style={style}
                ...
                <div onMouseDown={this.onDragStart__chart} ref="statistic"
                     className="dashboarddWidget__Statistic dayStatChartWrap">
                    <div onClick={this.onClickButton__chart(-1)} ref="left" className="button left active">
                        <div className="chart--button__left arrow left">{arrow()}</div>
                    </div>
                    <div onClick={this.onClickButton__chart(1)} ref="right" className="button right active">
                        <div className="chart--button__right arrow right">{arrow()}</div>
                    </div>
                </div>
    ...
    return (
        <svg width="20" height="21" viewBox="0 0 20 21" xmlns="http://www.w3.org/2000/svg">
            <g fill="none" fillRule="evenodd">
                ...
                <path
                    d="M5.848 18.125c.461.5 1.213.5 1.676 0l6.629-7.22a1.35 1.35 0 0 0 0-1.81l-6.629-7.22c-.463-.5-1.213-.5-1.676 0a1.352 1.352 0 0 0-.002 1.809L11.734 10l-5.886 6.316a1.35 1.35 0 0 0 0 1.809z"
                    fill="#616574"/>
            </g>
        </svg>
    )
